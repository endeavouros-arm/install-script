...
function sway() {
   ...
   message="\nInstalling Sway WM  "
   pacman -S --noconfirm --needed - < sway-pkg-list
   ok_nok  # function call
   # configure Sway
   cd /home/$username
   sudo -u $username mkdir /home/$username/.config
...
#################################################################################################
   ...
   if [ "$installtype" == "desktop" ]
   then
   dename=$(whiptail --nocancel --title "EndeavourOS ARM Setup - Desktop Selection" --menu --notags "Choose which Desktop Environment to install" 17 100 10 \
